...
import { processIDPhoto, determineAIModel } from "../services/geminiService";
...
const ImageEditor: React.FC<ImageEditorProps> = ({
  ...
  const pendingModelInfo = useMemo(() => {
    if (!hasPendingAiChanges) return null;
    return determineAIModel(
      settings.background,
      settings.backgroundHex,
  ...
  }, [hasPendingAiChanges, settings, config]);

  const handleApplyAiChanges = useCallback(async (): Promise<string> => {
    const currentHash = getAiSettingsHash(settings);
    ...
    try {
      ...
      if (isDefault) {
        ...
        const modelDecision = determineAIModel(
          settings.background,
          settings.backgroundHex,
          settings.clothingPrompt,
          resolvedBeauty,
        ...
        setLoadingAction(`Đang xử lý (${modelDecision.model})...`);
        ...
        const modelBadge = modelDecision.model.includes("pro") ? "Pro Model" : "Flash Model";
        addToHistory(result, `AI Processed (${modelBadge})`);
      }
    ...
    return resultUrl;
  }, [settings, baseImage, getAiSettingsHash, config, addToHistory, processedUrl]);
  ...
  const handleFinish = () => {
    ...
    if (hasPendingAiChanges) {
      if (confirm("Có thay đổi chưa áp dụng. Áp dụng ngay?")) {
        handleApplyAiChanges().then((latestProcessedUrl) => {
          applyClientAdjustments(latestProcessedUrl, settings, (finalUrl) => {
        ...
        <div className="p-4 lg:p-6 bg-dark-800 border-t border-white/10 space-y-3">
          {hasPendingAiChanges && (
            <button
              ...
              className="w-full py-3.5 bg-brand-600 hover:bg-brand-500 text-white rounded-2xl font-black text-sm uppercase tracking-[0.15em] shadow-lg shadow-brand-500/30 active:scale-95 transition-all flex flex-col items-center justify-center gap-0.5 group"
            >
              <div className="flex items-center gap-2">
                ...
                <span>{t("editor.btn.apply", config)}</span>
              </div>
              {pendingModelInfo && (
                <span className="text-[10px] font-mono font-bold text-white/80 normal-case tracking-normal">
                  Model: {pendingModelInfo.model}
                </span>
              )}
            </button>
          )}
