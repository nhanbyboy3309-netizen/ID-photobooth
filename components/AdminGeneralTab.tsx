...
import { InputGroup, iconInputClass, ToggleRow } from './AdminShared';
...
const AdminGeneralTab: React.FC<AdminGeneralTabProps> = ({ form, onChange }) => {
  ...
  return (
    <div className="space-y-6 animate-fadeIn text-gray-900 dark:text-white">
      ...
      {/* Print Sheet Footer Toggles */}
      <div className="bg-white dark:bg-gray-900 p-6 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-800">
        <h3 className="text-lg font-black text-gray-900 dark:text-white mb-1 border-b border-gray-100 dark:border-gray-800 pb-2">
          Chân trang phiếu in
        </h3>
        <p className="text-xs text-gray-500 dark:text-gray-400 mb-5 mt-3">
          Bật/tắt Logo và mã QR hiển thị ở dải chân trang trên mỗi tờ ảnh in (khổ 13x18).
        </p>
        <div className="space-y-3">
          <ToggleRow
            label="Hiển thị Logo"
            description="Logo cửa hàng ở góc trái chân trang phiếu in"
            icon="🖼️"
            checked={form.printShowLogo !== false}
            onChange={(v) => onChange('printShowLogo', v)}
          />
          <ToggleRow
            label="Hiển thị mã QR"
            description="Mã QR để khách quét lại tải ảnh, đặt ở góc phải chân trang"
            icon="🔳"
            checked={form.printShowQr !== false}
            onChange={(v) => onChange('printShowQr', v)}
          />
        </div>
      </div>
    </div>
