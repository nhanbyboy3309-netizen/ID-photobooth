...
const AdminTestingTab: React.FC<AdminTestingTabProps> = ({ form }) => {
  ...
  const [testBaseImage, setTestBaseImage] = useState<string>(SAMPLE_IMAGE);
  const [testProcessedImage, setTestProcessedImage] = useState<string>(SAMPLE_IMAGE);
  ...
  return (
    <div className="space-y-6 animate-fadeIn">
      <div className="bg-white dark:bg-gray-900 p-6 rounded-3xl shadow-sm border border-gray-200 dark:border-gray-800">
        <h2 className="text-xl font-black tracking-tight text-gray-900 dark:text-white mb-6 flex items-center gap-2">
           <span className="p-2 bg-brand-50 dark:bg-brand-950/40 text-brand-600 dark:text-brand-400 rounded-xl text-lg">🧪</span>
           Kiểm tra hệ thống
        </h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="p-6 border border-gray-200 dark:border-gray-800 rounded-2xl bg-gray-50/50 dark:bg-gray-800/50 space-y-4 hover:shadow-md hover:-translate-y-0.5 transition-all group">
            <div className="text-4xl group-hover:scale-110 transition-transform duration-300">📷</div>
            <div>
               <h3 className="font-bold text-gray-900 dark:text-white">Camera & AI Biometric</h3>
               <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">Kiểm tra khả năng nhận diện mặt và hướng dẫn của AI.</p>
            </div>
            <button onClick={() => setTestView('camera')} className="w-full py-3 bg-brand-600 text-white rounded-xl text-sm font-bold hover:bg-brand-700 transition shadow-lg shadow-brand-500/20 active:scale-95">Mở Camera Test</button>
          </div>
          <div className="p-6 border border-gray-200 dark:border-gray-800 rounded-2xl bg-gray-50/50 dark:bg-gray-800/50 space-y-4 hover:shadow-md hover:-translate-y-0.5 transition-all group">
            <div className="text-4xl group-hover:scale-110 transition-transform duration-300">✨</div>
            <div>
               <h3 className="font-bold text-gray-900 dark:text-white">Trình biên tập AI</h3>
               <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">Kiểm tra tính năng tách nền, thay áo và makeup AI.</p>
            </div>
            <button onClick={() => setTestView('editor')} className="w-full py-3 bg-purple-600 text-white rounded-xl text-sm font-bold hover:bg-purple-700 transition shadow-lg shadow-purple-500/20 active:scale-95">Mở Editor Test</button>
          </div>
          <div className="p-6 border border-gray-200 dark:border-gray-800 rounded-2xl bg-gray-50/50 dark:bg-gray-800/50 space-y-4 hover:shadow-md hover:-translate-y-0.5 transition-all group">
            <div className="text-4xl group-hover:scale-110 transition-transform duration-300">🖨️</div>
            <div>
               <h3 className="font-bold text-gray-900 dark:text-white">In ấn & QR</h3>
               <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">Kiểm tra dàn trang in 13x18 và tạo mã QR Cloud.</p>
            </div>
            <button onClick={() => setTestView('print')} className="w-full py-3 bg-indigo-600 text-white rounded-xl text-sm font-bold hover:bg-indigo-700 transition shadow-lg shadow-indigo-500/20 active:scale-95">Mở Print Test</button>
          </div>
        </div>
      </div>
          ...
          <div className="flex-1 w-full h-full">
            ...
            {testView === 'editor' && (
               <ImageEditor
                  ...
                  initialBaseImage={testBaseImage}
                  initialProcessedImage={testProcessedImage}
                  initialHistory={testHistory}
                  settings={testSettings}
                  onUpdateSettings={setTestSettings}
                  onProcessedImage={setTestProcessedImage}
                  onUpdateBaseImage={setTestBaseImage}
                  onUpdateHistory={setTestHistory}
                  onNext={() => {}}
            ...
            {testView === 'print' && (
               <PrintPreview
                  photoId="TEST0000"
                  processedImage={testProcessedImage || SAMPLE_IMAGE}
